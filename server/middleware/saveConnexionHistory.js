...
const saveConnexionHistory = async (req) => {
	try {
		...
		const previousConnections = await HistoriqueConnexion.find({
			userId: req.userId,
		})
			.sort({ date: -1 })
			.limit(1);
		...
		if (previousConnections.length > 0) {
			...
			if (previousLocation !== location) {
				isSuspicious = true;
				console.log(
					`ALERTE : Connexion suspecte détectée pour l'utilisateur ${req.userId}`
				);
			}
		}
		...
		if (isSuspicious && req.userEmail) {
			...
			try {
				await sendAlertEmail(req.userEmail, location);
